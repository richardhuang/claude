...
def find_latest_file(directory_path=None):
    """Find the latest modified file in the given directory, or auto-detect Claude project dir"""
    # Auto-detect Claude project directory if not provided
    if directory_path is None:
        directory_path = find_claude_project_dir()
        if directory_path is None:
            raise FileNotFoundError("Cannot find Claude project directory. Please specify a path.")
...
def find_claude_project_dir():
    """Find the Claude project directory automatically."""
    home = Path.home()
...
def tail_follow(file_path=None, directory_path=None):
    ...
    Automatically detects if Claude creates a new log file and switches to it.
    """
...
def main():
    ...
    if args.follow:
        ...
        if args.logfile:
            ...
            # Auto-detect
            file_path = None
            directory_path = args.directory or find_claude_project_dir()
            if directory_path is None:
                print("Error: Cannot find Claude project directory.")
                sys.exit(1)
        ...
        if not os.path.exists(file_path) if file_path else True:
            ...
            test_dir = directory_path or (find_claude_project_dir() if not args.directory else args.directory)
            if test_dir:
